describe('Portfolio tests', () => {
	...
	it('should page have actions buttons with correct link', () => {
		cy.getBySelector('email').should('have.attr', 'href', Cypress.env('URL_EMAIL'))
		...
		cy.getBySelector('download').should('have.attr', 'href')

		cy.getBySelector('linkedin').should('have.attr', 'href', Cypress.env('URL_LINKEDIN'))
	})

	it('should have css classes when initial render and when click in other experience', () => {
		cy.getBySelector('experience-0').should('have.class', 'bg-dark-600 max-lg:border-b-2 border-brand lg:border-l-2')
		cy.contains('Frontend Developer React').should('be.visible')

		cy.getBySelector('experience-2')
			.click()
			.should('have.class', 'bg-dark-600 max-lg:border-b-2 border-brand lg:border-l-2')
